package mx.gob.imss.cit.mjlssc.model.entity;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;
import mx.gob.imss.cit.mjlssc.persistence.entity.MjlcTipoFlujoExcepcion;

import javax.validation.constraints.NotNull;
import javax.validation.constraints.Size;
import java.io.Serializable;
import java.util.Date;

/**
 * A DTO for the {@link MjlcTipoFlujoExcepcion} entity
 */
@Data
@AllArgsConstructor
@NoArgsConstructor
public class MjlcTipoFlujoExcepcionDto implements Serializable {
    /**
	 * 
	 */
	private static final long serialVersionUID = -8977229342848720023L;
	private Integer id;
    private SsccTipoAsuntoDto cveTipoAsunto;
    @Size(max = 100)
    private String desTipoFlujoExcepcion;
    private Date fecAlta;
    private Date fecBaja;
    private Date fecModifica;
    @Size(max = 60)
    @NotNull
    private String cveUsuarioAlta;
    @Size(max = 60)
    private String cveUsuarioBaja;
    @Size(max = 60)
    private String cveUsuarioModifica;
}